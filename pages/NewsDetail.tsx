  ...
  return (
    <div className="max-w-4xl mx-auto p-4 lg:p-8">
      <div className="flex items-center justify-between mb-6">
        ...
        <button onClick={handleShare} className="text-blue-600 hover:bg-blue-50 p-2 rounded-full transition-colors">
            <Share2 className="w-5 h-5" />
        </button>
      </div>

      <article className="bg-white rounded-2xl shadow-sm border overflow-hidden mb-8">
        <img src={news.image} alt={news.title} className="w-full h-64 lg:h-96 object-cover" />
        <div className="p-6 lg:p-10">
          <div className="flex items-center gap-4 text-sm text-gray-500 mb-4">
             <span className="bg-blue-100 text-blue-800 px-2 py-1 rounded-md font-medium">{news.category}</span>
          ...
          <h1 className="text-3xl lg:text-4xl font-bold text-gray-900 mb-6 leading-tight">{news.title}</h1>
      ...
      <div className="bg-white rounded-2xl shadow-sm border p-6 lg:p-8">
         ...
         <div className="space-y-6">
            ...
            {comments.length === 0 && <p className="text-gray-500 text-sm">Пока нет комментариев.</p>}
         </div>
      </div>
    </div>
  );
