        ...
        <div className="max-w-4xl mx-auto p-4 lg:p-8 pb-24">
            ...
            <div className="flex border-b dark:border-gray-700 mb-6 overflow-x-auto">
                <button 
                    ...
                    className={`px-6 py-3 font-medium transition-colors border-b-2 whitespace-nowrap flex items-center gap-2 ${activeTab === 'available' ? 'border-green-600 text-green-600 dark:text-green-400' : 'border-transparent text-gray-500 dark:text-gray-400'}`}
                >
                    <Package className="w-4 h-4" /> Доступные ({availableOrders.length})
                </button>
                <button 
                    onClick={() => setActiveTab('active')}
                    className={`px-6 py-3 font-medium transition-colors border-b-2 whitespace-nowrap flex items-center gap-2 ${activeTab === 'active' ? 'border-green-600 text-green-600 dark:text-green-400' : 'border-transparent text-gray-500 dark:text-gray-400'}`}
                >
                    <Navigation className="w-4 h-4" /> В работе ({activeOrders.length})
                </button>
                <button 
                    onClick={() => setActiveTab('history')}
                    className={`px-6 py-3 font-medium transition-colors border-b-2 whitespace-nowrap flex items-center gap-2 ${activeTab === 'history' ? 'border-green-600 text-green-600 dark:text-green-400' : 'border-transparent text-gray-500 dark:text-gray-400'}`}
                >
            ...
            <div className="space-y-4">
                {activeTab === 'available' && (
                    ...
                    availableOrders.map(order => (
                        <div key={order.id} className="bg-white dark:bg-gray-800 p-6 rounded-xl border dark:border-gray-700 shadow-sm hover:shadow-md transition-all">
                            <div className="flex justify-between items-start mb-4">
                                <div>
                                    <h3 className="font-bold text-lg dark:text-white">{order.businessName}</h3>
                                    <p className="text-gray-500 dark:text-gray-400 text-sm flex items-center gap-1"><MapPin className="w-3 h-3" /> {order.businessAddress || 'Центр'}</p>
                                </div>
                            ...
                            <div className="border-l-2 border-gray-200 dark:border-gray-600 pl-4 py-2 my-4 relative">
                                <div className="absolute -left-[5px] top-0 w-2 h-2 rounded-full bg-gray-300 dark:bg-gray-500"></div>
                ...
                {activeTab === 'active' && (
                    ...
                    activeOrders.map(order => (
                        <div key={order.id} className="bg-white dark:bg-gray-800 p-6 rounded-xl border-2 border-green-500 shadow-md">
                            ...
                            <div className="grid md:grid-cols-2 gap-6 mb-6">
                                <div className="bg-gray-50 dark:bg-gray-700/50 p-4 rounded-lg">
                                    <p className="text-xs text-gray-500 dark:text-gray-400 mb-1">ЗАБРАТЬ ИЗ</p>
                                    <p className="font-bold text-lg dark:text-white">{order.businessName}</p>
                                    <p className="text-sm text-gray-600 dark:text-gray-300">{order.businessAddress}</p>
                                </div>
                                <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-lg">
                                    <p className="text-xs text-blue-500 dark:text-blue-400 mb-1">ОТВЕЗТИ КЛИЕНТУ</p>
                                    <p className="font-bold text-lg text-blue-900 dark:text-blue-300">{order.address}</p>
                                    <Button variant="outline" size="sm" className="mt-2 text-xs h-7 bg-white dark:bg-gray-700 dark:border-gray-600 dark:text-white">Показать на карте</Button>
                                </div>
                            </div>
                ...
                {activeTab === 'history' && (
                    ...
                    historyOrders.map(order => (
                        <div key={order.id} className="bg-white dark:bg-gray-800 p-4 rounded-xl border dark:border-gray-700 flex items-center justify-between opacity-70">
                            <div>
                                <p className="font-bold text-gray-900 dark:text-white">{order.businessName} - {order.address}</p>
