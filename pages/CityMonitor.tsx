        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
            <div className="bg-white rounded-xl w-full max-w-lg p-6 shadow-2xl h-[90vh] overflow-y-auto">
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div>
                        <label className="text-sm font-medium text-gray-700">Фото проблемы</label>
    ...
    return (
        <div className="max-w-6xl mx-auto p-4 lg:p-8">
            ...
            {resolvingId && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
                    <div className="bg-white rounded-xl w-full max-w-sm p-6 shadow-2xl">
                        <h3 className="font-bold text-lg mb-4">Подтверждение решения</h3>
                        <p className="text-sm text-gray-500 mb-4">Загрузите фото результата, чтобы закрыть заявку.</p>
                        ...
                        <div className="border-2 border-dashed border-gray-300 rounded-lg p-4 text-center mb-4">
                            {resultImage ? (
                                ...
                                <div className="relative cursor-pointer">
                                    ...
                                    <span className="text-sm text-gray-500">{uploadingResult ? "Загрузка..." : "Фото результата"}</span>
                        ...
                        <div className="flex gap-2">
                            <Button variant="outline" className="flex-1" onClick={() => { setResolvingId(null); setResultImage(''); }}>Отмена</Button>
            ...
            <div className="flex items-center gap-4 mb-6">
                 <h2 className="text-xl font-bold text-gray-900">Лента обращений</h2>
                 <span className="bg-gray-100 text-gray-600 px-2 py-0.5 rounded-full text-sm font-bold">{appeals.length}</span>
            </div>
            ...
            {loading ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {appeals.map(appeal => (
                        <div key={appeal.id} className={`bg-white rounded-2xl border shadow-sm overflow-hidden flex flex-col ${appeal.status === 'done' ? 'ring-2 ring-green-500 ring-offset-2' : ''}`}>
                            ...
                            <div className="p-5 flex-1 flex flex-col">
                                <h3 className="font-bold text-lg mb-2 line-clamp-1">{appeal.title}</h3>
                                <div className="flex items-center text-gray-500 text-sm mb-3">
                                    <MapPin className="w-3.5 h-3.5 mr-1" /> {appeal.location}
                                </div>
                                <p className="text-gray-600 text-sm mb-4 line-clamp-3 flex-1">{appeal.description}</p>
                                
                                {appeal.status === 'done' ? (
                                    <div className="bg-green-50 text-green-700 p-3 rounded-lg flex items-center gap-3 text-sm font-medium">
                                        <CheckCircle2 className="w-5 h-5" />
                                        Проблема устранена!
                                    </div>
                                ) : (
                                    <div className="bg-red-50 text-red-700 p-3 rounded-lg flex items-center gap-3 text-sm font-medium">
