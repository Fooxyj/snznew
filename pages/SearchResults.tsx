                        ...
                        {results.businesses.map(biz => (
                            <div key={biz.id} className="bg-white dark:bg-gray-800 p-4 rounded-xl border dark:border-gray-700 shadow-sm flex gap-4 hover:shadow-md transition-shadow">
                                ...
                                <div>
                                    ...
                                    <p className="text-sm text-gray-500 dark:text-gray-400 mt-1 flex items-center"><MapPin className="w-3 h-3 mr-1" /> {biz.address}</p>
                                </div>
            ...
            {results.ads.length > 0 && (
                <div className="mb-10">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {results.ads.map(ad => (
                            <div key={ad.id} className="bg-white dark:bg-gray-800 p-3 rounded-xl border dark:border-gray-700 shadow-sm flex gap-3 hover:shadow-md transition-shadow">
                                <img src={ad.image} alt="" className="w-24 h-24 rounded-lg object-cover bg-gray-100 dark:bg-gray-700 shrink-0" />
                                <div className="flex-1 min-w-0">
                                    <h4 className="font-medium truncate dark:text-white">{ad.title}</h4>
                                    <p className="font-bold text-blue-600 dark:text-blue-400">{ad.price.toLocaleString()} {ad.currency}</p>
                     ...
                     <div className="mt-4">
                        <Link to="/classifieds" className="text-blue-600 dark:text-blue-400 text-sm hover:underline">Перейти в объявления →</Link>
                    </div>
                </div>
            )}
                    ...
                    <div className="space-y-4">
                        {results.news.map(n => (
                            <Link key={n.id} to={`/news/${n.id}`} className="block bg-white dark:bg-gray-800 p-4 rounded-xl border dark:border-gray-700 shadow-sm hover:shadow-md transition-shadow">
                                ...
                                <h3 className="font-bold text-lg mb-1 dark:text-white">{n.title}</h3>
                                <p className="text-gray-500 dark:text-gray-400 text-sm line-clamp-2">{n.content}</p>
                            </Link>
                        ))}
                    </div>
                </div>
