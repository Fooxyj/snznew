      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl w-full max-w-lg shadow-2xl overflow-hidden slide-in-from-top h-[90vh] md:h-[85vh] flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto p-6 custom-scrollbar">
            ...
            <div>
                ...
                <input required type="text" className="w-full px-3 py-2 border rounded-lg outline-none bg-white dark:bg-gray-700 border-gray-200 dark:border-gray-600 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500" value={formData.title} onChange={e => setFormData({...formData, title: e.target.value})} />
            </div>
            <div className="grid grid-cols-2 gap-4">
                <div>
                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Цена (₽)</label>
                    <input required type="number" className="w-full px-3 py-2 border rounded-lg outline-none bg-white dark:bg-gray-700 border-gray-200 dark:border-gray-600 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500" value={formData.price} onChange={e => setFormData({...formData, price: e.target.value})} />
                </div>
                <div>
                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Категория</label>
                    <select className="w-full px-3 py-2 border rounded-lg outline-none bg-white dark:bg-gray-700 border-gray-200 dark:border-gray-600 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500" value={formData.category} onChange={e => setFormData({...formData, category: e.target.value})}>
            ...
            <div>
                ...
                <textarea required rows={4} className="w-full px-3 py-2 border rounded-lg outline-none resize-none bg-white dark:bg-gray-700 border-gray-200 dark:border-gray-600 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500" value={formData.description} onChange={e => setFormData({...formData, description: e.target.value})} />
            </div>
            <div>
                ...
                <div className="grid grid-cols-3 gap-2">
                    {images.map((img, idx) => (
                        <div key={idx} className="relative aspect-square rounded-lg overflow-hidden border dark:border-gray-600 group bg-gray-100 dark:bg-gray-700">
                            ...
                            {idx === 0 && <div className="absolute bottom-0 left-0 right-0 bg-black/50 text-white text-[10px] text-center py-0.5">Главное</div>}
                        </div>
                    ))}
                    {images.length < 5 && (
                        <div className="relative aspect-square rounded-lg border-2 border-dashed border-gray-300 dark:border-gray-600 flex flex-col items-center justify-center text-gray-400 hover:bg-gray-50 transition-colors cursor-pointer bg-white dark:bg-transparent">
                            {isUploading ? <Loader2 className="w-6 h-6 animate-spin text-blue-500" /> : <><Plus className="w-6 h-6 mb-1" /><span className="text-xs">Добавить</span></>}
        ...
        <div className="px-6 py-4 border-t dark:border-gray-700 bg-gray-50 dark:bg-gray-800 shrink-0">
            <Button form="edit-ad-form" disabled={isLoading || isUploading} className="w-full py-3">
