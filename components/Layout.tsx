...
export const Layout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  return (
    <div className="h-screen bg-white dark:bg-gray-900 flex overflow-hidden transition-colors duration-200">
      ...
      <aside className={`fixed lg:static inset-y-0 left-0 z-[110] w-72 bg-white dark:bg-gray-800 border-r dark:border-gray-800 transform transition-transform duration-300 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'} flex flex-col shadow-2xl lg:shadow-none`}>
        <div className="p-6 flex items-center justify-between shrink-0">
          ...
          <button onClick={closeSidebar} className="p-2 text-gray-400 hover:text-gray-600 dark:hover:text-white transition-colors">
            <X className="w-6 h-6" />
          </button>
        </div>
