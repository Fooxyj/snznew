...
export const AdDetailsModal: React.FC<AdDetailsModalProps> = ({ ad, isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 bg-dark/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 overflow-y-auto" onClick={onClose}>
      <div className="bg-surface w-full max-w-4xl rounded-3xl shadow-2xl relative overflow-hidden animate-fade-in-up flex flex-col md:flex-row max-h-[90vh]" onClick={e => e.stopPropagation()}>
        ...
        <div className="w-full md:w-1/2 flex flex-col h-full bg-surface">
            <div className="p-6 md:p-8 overflow-y-auto flex-grow custom-scrollbar">
                ...
                <div className="mb-6">
                    ...
                    <h2 className="text-2xl md:text-3xl font-bold text-dark leading-tight mb-3">{ad.title}</h2>
            ...
            <div className="p-6 border-t border-gray-100 bg-white z-10">
                <div className="bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-100 rounded-2xl p-4 flex items-center justify-between shadow-sm">
                    <div>
                        <span className="block text-xs text-primary font-bold uppercase mb-1">Контакты продавца</span>
                        <span className="block text-xl md:text-2xl font-bold text-dark tracking-tight">{ad.contact}</span>
                    </div>
                    <a href={`tel:${ad.contact}`} className="flex items-center gap-2 bg-primary hover:bg-primary-dark text-white px-6 py-3 rounded-xl transition-all shadow-lg shadow-primary/20 hover:shadow-xl active:scale-95">
