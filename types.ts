...
export interface Achievement {
  ...
  description: string;
  icon: string;
  category: 'social' | 'activity' | 'reputation' | 'special';
  goal: number;
  current: number;
  isUnlocked: boolean;
}
...
export interface HeaderEvent { id: string; title: string; date: string; image: string; location: string; category: string; description?: string; price?: number; sessions?: string[]; authorId?: string; }
export interface Event extends HeaderEvent {}
...
export interface Notification { 
  ...
  isRead: boolean; 
  createdAt: string; 
  type?: 'system' | 'personal' | 'reward' | 'important';
  link?: string | null;
}
...
export interface Review { id: string; businessId: string; authorName: string; authorAvatar?: string; rating: number; text: string; date: string; authorId?: string; userId?: string; }
export interface Comment { id: string; newsId: string; authorName: string; authorAvatar?: string; text: string; date: string; }
export interface Conversation { id: string; participant1Id: string; participant2Id: string; partnerId: string; partnerName: string; partnerAvatar: string; lastMessageDate: string; lastMessageText: string; lastMessageDateRaw?: string; businessId?: string; businessName?: string; businessOwnerId?: string; unreadCount?: number; }
export interface Message { id: string; conversationId: string; senderId: string; text: string; createdAt: string; isRead: boolean; status?: 'pending' | 'sent' | 'read'; imageUrl?: string; audioUrl?: string; }
export interface CommunityPost { id: string; communityId: string; authorId: string; authorName: string; authorAvatar?: string; content: string; image?: string; likes: number; createdAt: string; status?: 'pending' | 'approved' | 'rejected'; isLiked?: boolean; }
...
export interface ExclusivePage { id: string; title: string; description: string; image_url: string; phone?: string; website?: string; features: { t: string; d: string; i: string }[]; blocks_config: PageBlock[]; is_active: boolean; idx: number; }
export interface Banner { id: string; idx?: number; image_url: string; link_url?: string; title?: string; position?: string; is_active: boolean; created_at: string; erid?: string; advertiser_info?: string; }
export interface PromoAd { id: string; title: string; description: string; image_url: string; link_url?: string; price?: number; category: string; is_active: boolean; created_at: string; erid?: string; advertiser_info?: string; }
